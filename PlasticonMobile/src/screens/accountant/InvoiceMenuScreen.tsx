...
export function InvoiceMenuScreen() {
  ...
  const ITEMS = [
    { icon: 'document-text', label: 'الفواتير',                         desc: 'عرض وإدارة جميع فواتير العملاء',     screen: 'Invoices',            color: colors.primary },
    { icon: 'people',        label: 'ذمم العملاء',           desc: 'تتبع المدفوعات المستحقة من العملاء', screen: 'CustomerReceivables', color: colors.info },
    { icon: 'business',      label: 'ذمم الموردين',             desc: 'مراقبة المبالغ المستحقة للموردين',          screen: 'SupplierPayables',    color: colors.warning },
  ];

  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        <Text style={[styles.title, { color: colors.text }]}>{'الفواتير'}</Text>
        <Text style={[styles.sub, { color: colors.textMuted }]}>{'إدارة الفواتير وتتبع المدفوعات'}</Text>
