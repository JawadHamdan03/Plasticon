...
import { CheckSquare, Plus, AlertTriangle, CheckCircle, Clock, Activity } from "lucide-react";
...
const SEVERITIES: QualityCheck["severity"][] = ["LOW", "MEDIUM", "HIGH", "CRITICAL"];
...
export function QualityChecksPage() {
  ...
  const t = {
    ...
    subtitle:    isAr ? "تسجيل ومتابعة مشاكل الجودة في خط الإنتاج" : "Log and track quality issues on the production line",
  ...
  const submit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(await readApiError(res));
      setSuccess(isAr ? "تم حفظ فحص الجودة بنجاح" : "Quality check saved successfully");
    ...
    <div className="maintenance-page">
      ...
      <div className="maintenance-kpi-strip">
        ...
        <div className="maintenance-kpi">
          ...
          <span className="maintenance-kpi__value" style={{ color: openCount > 0 ? "var(--orange-500)" : undefined }}>{openCount}</span>
        </div>
        <div className="maintenance-kpi">
          <span className="maintenance-kpi__label">{t.kpiCritical}</span>
          <span className="maintenance-kpi__value" style={{ color: criticalCount > 0 ? "var(--red-600)" : undefined }}>{criticalCount}</span>
        </div>
        <div className="maintenance-kpi">
          <span className="maintenance-kpi__label">{t.kpiResolved}</span>
          <span className="maintenance-kpi__value" style={{ color: "var(--green-600)" }}>{resolvedCount}</span>
        </div>
      </div>
      ...
      <div className="maintenance-grid">
          ...
          <div className="maintenance-card__body" style={{ display: "grid", gap: ".625rem" }}>
            {loading ? (
              ...
              <div style={{ padding: "2.5rem", textAlign: "center" }}>
                <Activity size={36} style={{ color: "var(--gray-300)", margin: "0 auto .75rem", display: "block" }} />
              ...
              <div key={r.id} className="maintenance-report-item">
                <div className="maintenance-report-item__header">
                  <span className="maintenance-report-item__machine">{r.machine?.name ?? `#${r.id}`}</span>
                  <span className="maintenance-report-item__date">{fmtDate(r.createdAt)}</span>
                </div>
                <p className="maintenance-report-item__parts">
                  {isAr ? "نوع: " : "Type: "}<strong>{r.issueType.replace(/_/g, " ")}</strong>
                </p>
                <div className="maintenance-report-item__meta">
                  <span className={severityClass(r.severity)}>{r.severity}</span>
                  {r.resolvedAt ? (
                    <span style={{ fontSize: ".72rem", color: "var(--green-600)", display: "flex", alignItems: "center", gap: ".2rem" }}>
                      <CheckCircle size={11} /> {t.resolved}
                    </span>
                  ) : (
                    <span style={{ fontSize: ".72rem", color: "var(--orange-500)", display: "flex", alignItems: "center", gap: ".2rem" }}>
                ...
                {r.description && (
                  <p style={{ margin: ".375rem 0 0", fontSize: ".82rem", color: "var(--text-secondary)", background: "var(--bg-surface)", padding: ".5rem .75rem", borderRadius: "var(--radius-md)", border: "1px solid var(--border-default)", lineHeight: 1.55 }}>
                    {r.description}
                  </p>
                )}
              </div>
        ...
        {showForm ? (
            ...
            <div className="maintenance-card__body">
              <form className="module-form" onSubmit={(e) => void submit(e)}>
                ...
                <label>
                  ...
                  <select
                    value={form.severity}
                    onChange={e => setForm(p => ({ ...p, severity: e.target.value as QualityCheck["severity"] }))}
                  >
                    {SEVERITIES.map(s => (
                ...
                <label>
                  ...
                  <textarea
                    ...
                    placeholder={isAr ? "وصف المشكلة والإجراءات المتخذة..." : "Describe the issue and actions taken..."}
                  />
                </label>

                <div style={{ display: "flex", gap: ".625rem" }}>
                  <button type="submit" className="auth-button" style={{ flex: 1 }} disabled={submitting}>
                    {submitting ? (isAr ? "جارٍ الحفظ..." : "Saving...") : t.save}
                  </button>
          ...
          <div
            ...
            style={{ display: "grid", placeItems: "center", minHeight: "200px", cursor: "pointer", border: "2px dashed var(--border-default)", background: "transparent", boxShadow: "none" }}
            onClick={() => setShowForm(true)}
          >
            <div style={{ textAlign: "center", color: "var(--text-secondary)" }}>
              <Plus size={28} style={{ margin: "0 auto .5rem", color: "var(--brand-primary)", display: "block" }} />
              <p style={{ margin: 0, fontWeight: 600, fontSize: ".875rem" }}>{t.newCheck}</p>
              <p style={{ margin: ".25rem 0 0", fontSize: ".78rem" }}>
                {isAr ? "انقر لتسجيل فحص جودة جديد" : "Click to log a new quality check"}
              </p>
            </div>
          </div>
        )}
