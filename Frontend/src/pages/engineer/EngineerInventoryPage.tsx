...
import {
  Plus, Trash2, Send, Package, ChevronDown, ChevronUp,
  CheckCircle, Clock, Eye, Image, X, AlertCircle,
} from "lucide-react";
...
export function EngineerInventoryPage() {
  ...
  const handleAddItem = async (inventoryId: number) => {
    ...
    if (!newPart.trim()) { setAddItemError("Part name is required"); return; }
    if (!newQty || Number(newQty) < 1) { setAddItemError("Quantity must be at least 1"); return; }
    ...
    try {
      ...
      setNewPart(""); setNewQty(""); setNewImage(null);
  ...
  const handleSubmit = async (inventoryId: number) => {
    ...
    if (!confirm(`Submit this inventory for ${MONTHS[inv.month - 1]} ${inv.year}? This cannot be undone.`)) return;
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-header__sub">Submit monthly shortage reports — admin & accountant will be notified</p>
        </div>
        <button
      ...
      {showNewForm && (
        <div className="card">
          <div className="card-header">
            ...
            <button className="btn btn--ghost btn--icon" onClick={() => setShowNewForm(false)}>
              <X size={18} />
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="card">
          <div className="empty-state" style={{ padding: "3rem" }}>
            ...
            <p className="empty-state__desc">Create your first monthly parts shortage report.</p>
                  ...
                  <div className="card-body">
                    ...
                    {inv.items.length > 0 ? (
                      ...
                      <div className="empty-state" style={{ padding: "1.5rem", marginBottom: "1rem" }}>
                        <Package size={24} style={{ color: "var(--gray-300)" }} />
                        <p className="empty-state__desc">No parts added yet. Add parts below.</p>
                      </div>
                    )}
                    ...
                    {canEdit && (
                      <>
                        {addingItemToId === inv.id ? (
                          ...
                          >
                            <p style={{ margin: "0 0 .75rem", fontWeight: 700, fontSize: ".88rem", color: "var(--blue-800)" }}>
                              Add New Part
                            </p>
                            {addItemError && (
                              <div className="auth-alert auth-alert--error" style={{ marginBottom: ".75rem", padding: ".5rem .75rem", fontSize: ".8rem" }}>
                                {addItemError}
                              </div>
                            )}
                            ...
                            <div style={{ display: "flex", gap: ".625rem" }}>
                              ...
                              <button
                                ...
                                onClick={() => { setAddingItemToId(null); setAddItemError(""); setNewPart(""); setNewQty(""); setNewImage(null); }}
                              >
                                Cancel
                              </button>
                            </div>
                          ...
                          <button
                            ...
                            onClick={() => { setAddingItemToId(inv.id); setAddItemError(""); }}
                          >
