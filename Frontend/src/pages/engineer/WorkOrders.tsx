...
export default function WorkOrders() {
  ...
  const handleDelete = async (id: number) => {
    if (!(await confirmDialog("حذف أمر العمل هذا؟", { danger: true }))) return;
    await fetch(`${API_BASE_URL}/maintenance-schedule/${id}`, {
  ...
  /* ── KPI counts ────────────────────────────────────────────── */
  const kpis = [
    { label: "مهام اليوم", value: todayOrders.length,   gradient: "linear-gradient(135deg,#3b82f6,#1d4ed8)" },
    { label: "متأخرة",      value: overdueOrders.length, gradient: "linear-gradient(135deg,#ef4444,#dc2626)" },
    { label: "مكتملة",      value: doneOrders.length,    gradient: "linear-gradient(135deg,#10b981,#059669)" },
    { label: "الإجمالي",    value: orders.length,        gradient: "linear-gradient(135deg,#8b5cf6,#7c3aed)" },
  ];

  return (
    <ModulePageShell
      title={"أوامر العمل"}
      subtitle={"مهام الصيانة اليومية: التزييت، فحص القوالب، تنظيف التجاويف والمزيد"}
      icon={<Wrench size={22} />}
    >
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: ".75rem", marginBottom: "1rem" }}>
        ...
        <div style={{ display: "flex", gap: ".35rem", flexWrap: "wrap" }}>
          {([
            ["today",   "اليوم",       todayOrders.length],
            ["all",     "الكل",           orders.length],
            ["overdue", "متأخرة",     overdueOrders.length],
            ["done",    "مكتملة",   doneOrders.length],
      ...
      {showForm && !isAdmin && (
        <Card style={{ padding: "1.25rem", marginBottom: "1.25rem", border: "2px solid var(--orange-500,#f97316)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
            <h3 style={{ margin: 0, fontSize: ".95rem", fontWeight: 700 }}>{"أمر عمل جديد"}</h3>
          ...
          <form onSubmit={(e) => void handleCreate(e)}>
              ...
              {TASK_PRESETS.map(p => {
                ...
                return (
                  <button key={p.key} type="button"
                    ...
                    <div style={{ fontSize: ".68rem", fontWeight: 700, color: sel ? "var(--orange-600,#ea580c)" : "var(--text-primary)", lineHeight: 1.2 }}>{p.ar}</div>
                  </button>
                );
              })}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: ".875rem", marginBottom: ".875rem" }}>
              ...
              <label style={{ display: "flex", flexDirection: "column", gap: ".3rem", fontSize: ".83rem", fontWeight: 600 }}>
                ...
                <select className="input" value={form.frequency} onChange={e => setForm(f => ({ ...f, frequency: e.target.value }))}>
                  {Object.entries(FREQ_LABELS).map(([k, v]) => <option key={k} value={k}>{v.ar}</option>)}
                </select>
              </label>
            </div>
            ...
            <div style={{ display: "flex", gap: ".5rem", justifyContent: "flex-end" }}>
              ...
              <button type="submit" disabled={saving}
                ...
                {saving ? "جاري الحفظ…" : "إنشاء أمر العمل"}
              </button>
            </div>
          </form>
        </Card>
      ...
      {loading ? (
        ...
        <Card style={{ padding: "3rem", textAlign: "center" }}>
          ...
          <p style={{ margin: 0, color: "var(--text-secondary)", fontWeight: 600 }}>
            {filterTab === "today"   ? "لا توجد مهام مجدولة لليوم"
            : filterTab === "overdue" ? "لا توجد أوامر متأخرة — عمل رائع!"
            : filterTab === "done"    ? "لا توجد أوامر مكتملة بعد"
            : "لا توجد أوامر عمل. أنشئ واحدًا للبدء."}
          </p>
        </Card>
      ) : (
                ...
                <div style={{ padding: ".85rem 1rem", display: "flex", alignItems: "center", gap: ".75rem", flexWrap: "wrap" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: ".5rem", flexWrap: "wrap" }}>
                    <div style={{ textAlign: "center" }}>
                      <p style={{ margin: 0, fontSize: ".65rem", color: "var(--text-secondary)", fontWeight: 600 }}>{"الاستحقاق"}</p>
                      <p style={{ margin: 0, fontSize: ".78rem", fontWeight: 700 }}>{new Date(order.nextScheduledDate).toLocaleDateString()}</p>
                    </div>
                    <div style={{ textAlign: "center" }}>
                      <p style={{ margin: 0, fontSize: ".65rem", color: "var(--text-secondary)", fontWeight: 600 }}>{"التكرار"}</p>
                      <p style={{ margin: 0, fontSize: ".72rem", fontWeight: 600, color: "var(--text-secondary)" }}>{FREQ_LABELS[order.frequency]?.ar}</p>
                    </div>
                ...
                {expanded && (
                  <div style={{ borderTop: "1px solid var(--border-default)", padding: ".75rem 1rem", background: "var(--bg-surface)", display: "flex", flexDirection: "column", gap: ".4rem" }}>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: ".5rem", fontSize: ".8rem" }}>
                      <div>
                        <span style={{ color: "var(--text-secondary)", fontWeight: 600 }}>{"النوع:"} </span>
                        <span>{order.scheduleType}</span>
                      </div>
                      {order.lastScheduledDate && (
                        <div>
                          <span style={{ color: "var(--text-secondary)", fontWeight: 600 }}>{"آخر تنفيذ:"} </span>
                          <span>{new Date(order.lastScheduledDate).toLocaleDateString()}</span>
                        </div>
                      )}
                      {order.assignedEngineer && (
                        <div>
                          <span style={{ color: "var(--text-secondary)", fontWeight: 600 }}>{"مسند إلى:"} </span>
                          <span>{order.assignedEngineer.fullName}</span>
                        </div>
                      )}
                      {order.createdBy && (
                        <div>
                          <span style={{ color: "var(--text-secondary)", fontWeight: 600 }}>{"أنشأه:"} </span>
                          <span>{order.createdBy.fullName}</span>
                        </div>
                      )}
                      <div>
                        <span style={{ color: "var(--text-secondary)", fontWeight: 600 }}>{"تاريخ الإنشاء:"} </span>
                      ...
                      <div style={{ display: "flex", gap: ".5rem", marginTop: ".5rem", flexWrap: "wrap" }}>
                        {sts !== "COMPLETED" && sts !== "IN_PROGRESS" && (
                          <button type="button" onClick={() => void patchStatus(order.id, "IN_PROGRESS")}
                            ...
                            <PlayCircle size={14} /> {"تعيين قيد التنفيذ"}
                          </button>
                        )}
                        {sts !== "COMPLETED" && (
                          <button type="button" onClick={() => void patchStatus(order.id, "COMPLETED")}
                            ...
                            <CheckCircle size={14} /> {"تعيين مكتمل"}
                          </button>
                        )}
                        {sts === "COMPLETED" && (
