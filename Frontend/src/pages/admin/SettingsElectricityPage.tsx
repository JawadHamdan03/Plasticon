...
export function SettingsElectricityPage() {
  ...
  const handleSetPrice = async (e: FormEvent) => {
    ...
    try {
      ...
      setSuccess("تم تحديث سعر kWh بنجاح");
      setNewPrice(""); setPriceNotes("");
      void loadReport();
    } catch (e) {
      setError(e instanceof Error ? e.message : "فشل تحديث السعر");
    } finally {
      setSaving(false);
    }
  };
  ...
  const tabs = [
    { id: "report" as const, label: "تقرير الاستهلاك", icon: <BarChart3 size={15} /> },
    { id: "price" as const, label: "سعر kWh", icon: <Settings size={15} /> },
  ];

  return (
    <ModulePageShell
      title={"إدارة الكهرباء"}
      subtitle="مراقبة استهلاك الكهرباء حسب الشفت، وإدارة سعر kWh وعرض تقارير التكلفة."
      actions={
        view && (
          <span style={{ fontSize: ".8rem", fontWeight: 700, padding: ".3rem .75rem", borderRadius: "999px", background: "var(--bg-subtle)", border: "1px solid var(--border-default)", color: "var(--text-secondary)" }}>
            ...
            {"السعر الحالي"}: {fmtNum(view.currentKwhPrice)} ILS/kWh
          </span>
        )
      }
    >
        ...
        <>
          ...
          <Card className="p-4 mb-6">
            <div style={{ display: "flex", flexWrap: "wrap", gap: ".75rem", alignItems: "flex-end" }}>
              ...
              <button type="button" className="auth-button" onClick={() => void loadReport()}>{"تطبيق"}</button>
          ...
          {loading ? (
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
                {[
                  { label: "إجمالي القراءات", value: view.totalReadings, unit: "", icon: <BarChart3 className="w-5 h-5" />, grad: "from-blue-500 to-blue-700" },
                  { label: "إجمالي الاستهلاك", value: fmtNum(view.totalConsumption), unit: "kWh", icon: <Zap className="w-5 h-5" />, grad: "from-yellow-500 to-orange-600" },
                  { label: "إجمالي التكلفة", value: fmtNum(view.totalCost), unit: "ILS", icon: <TrendingUp className="w-5 h-5" />, grad: "from-green-500 to-emerald-700" },
                  {
                    label: "شفتات ناقصة",
              ...
              {view.days.length === 0 ? (
                <Card className="p-8" style={{ textAlign: "center" }}>
                  <p style={{ color: "var(--text-secondary)" }}>{"لا توجد بيانات ضمن الفترة المحددة."}</p>
                </Card>
              ) : (
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 0 }}>
                        {day.shifts.map((s) => (
                          <div key={s.id} style={{ padding: "1rem 1.25rem", borderRight: "1px solid var(--border-default)", borderBottom: "1px solid var(--border-default)" }}>
                            ...
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".25rem .5rem", fontSize: ".8rem", color: "var(--text-secondary)" }}>
                              <span>{"بداية"}</span><strong style={{ color: "var(--text-primary)" }}>{fmtNum(s.startReading)}</strong>
                              <span>{"نهاية"}</span><strong style={{ color: "var(--text-primary)" }}>{fmtNum(s.endReading)}</strong>
                              <span>{"الاستهلاك"}</span><strong style={{ color: "var(--brand-primary)" }}>{fmtNum(s.consumption)} kWh</strong>
                              <span>{"التكلفة"}</span><strong style={{ color: "#15803d" }}>{fmtNum(s.shiftCost)} ILS</strong>
                            </div>
                        ...
                        {Array.from({ length: Math.max(0, 3 - day.shifts.length) }).map((_, i) => (
                          <div key={`missing-${i}`} style={{ padding: "1rem 1.25rem", borderRight: "1px solid var(--border-default)", borderBottom: "1px solid var(--border-default)", opacity: .4 }}>
                            <p style={{ margin: 0, fontSize: ".8rem", color: "var(--text-muted)", fontStyle: "italic" }}>{"شفت غير مسجل"}</p>
                          </div>
                        ))}
                      </div>
                    </Card>
        ...
        <div className="layout-sidebar-form">
          ...
          <Card className="p-5">
            ...
            <form onSubmit={(e) => void handleSetPrice(e)}>
              <label style={{ display: "flex", flexDirection: "column", gap: ".3rem", marginBottom: "1rem" }}>
                <span style={{ fontSize: ".8rem", fontWeight: 600, color: "var(--text-secondary)" }}>{"سعر kWh الجديد (ILS) *"}</span>
              ...
              <label style={{ display: "flex", flexDirection: "column", gap: ".3rem", marginBottom: "1.25rem" }}>
                <span style={{ fontSize: ".8rem", fontWeight: 600, color: "var(--text-secondary)" }}>{"ملاحظات"}</span>
                <textarea className="module-form__input" rows={2} value={priceNotes} onChange={(e) => setPriceNotes(e.target.value)} placeholder={"سبب تغيير السعر…"} style={{ resize: "vertical" }} />
              </label>
            ...
            {priceHistory.length > 0 && (
              <div style={{ marginTop: "1rem", padding: ".75rem", borderRadius: "var(--radius-md)", background: "var(--bg-subtle)" }}>
                <p style={{ margin: "0 0 .25rem", fontSize: ".75rem", fontWeight: 700, color: "var(--text-secondary)" }}>{"السعر الحالي"}:</p>
                ...
                <p style={{ margin: "0.25rem 0 0", fontSize: ".75rem", color: "var(--text-muted)" }}>
                  {"ساري من"}: {new Date(priceHistory[0].effectiveFrom).toLocaleDateString()}
                </p>
              </div>
            )}
          </Card>
          ...
          <Card className="overflow-hidden">
            <div style={{ padding: "1rem 1.25rem .75rem", borderBottom: "1px solid var(--border-default)", display: "flex", alignItems: "center", gap: ".5rem" }}>
              ...
              <h3 style={{ margin: 0, fontSize: "1rem", fontWeight: 700, color: "var(--text-primary)" }}>{"سجل الأسعار"}</h3>
            </div>
            {priceHistory.length === 0 ? (
              <p style={{ padding: "2rem 1.25rem", color: "var(--text-secondary)", textAlign: "center" }}>{"لا يوجد سجل أسعار."}</p>
            ) : (
