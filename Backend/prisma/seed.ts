...
const defaultUsers: SeedUser[] = [
    ...
    },
    {
        ...
        username: "engineer1",
        phone: "0599000003",
        email: "engineer1@plasticon.local",
    ...
    },
    {
        ...
        phone: "0599000004",
        email: "accountant1@plasticon.local",
...
async function seedProductionSettings(adminId: number | null) {
    await prisma.productionSetting.upsert({
        ...
        create: { productType: ProductType.CAPS, piecesPerCarton: 10, updatedById: adminId },
    });

    await prisma.productionSetting.upsert({
        where: { productType: ProductType.PREFORM },
        update: { piecesPerCarton: 10, updatedById: adminId },
        create: { productType: ProductType.PREFORM, piecesPerCarton: 10, updatedById: adminId },
    });
...
async function seedMachines() {
    for (const machine of defaultMachines) {
        ...
        if (exists) {
            console.log(`Skipped existing machine: ${machine.name} (${machine.type})`);
            continue;
        }
        ...
        console.log(`Created machine #${created.id}: ${created.name} (${created.type})`);
    }
}

async function seedUsers() {
    const password = await bcrypt.hash(DEFAULT_TEST_PASSWORD, BCRYPT_SALT_ROUNDS);

    for (const user of defaultUsers) {
        const shift = user.shiftName
            ? await prisma.shift.findFirst({ where: { name: user.shiftName }, select: { id: true } })
            : null;
...
async function seedSampleProduction() {
    ...
    if (!worker || !capsMachine || !worker.shiftId || !capsSetting) {
        console.log("Skipped sample production record (missing user/shift/machine/settings).");
        return;
    }
