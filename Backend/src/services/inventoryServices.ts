...
export const createInventoryTransaction = async (
    ...
    if (!Number.isInteger(materialId) || materialId <= 0) {
        return { status: 400, message: "materialId is required and must be a positive integer" };
    }
    ...
    if (!Object.values(ReferenceType).includes(payload.referenceType as ReferenceType)) {
    ...
    if (payload.type === InventoryType.OUT && material.currentQuantity < quantity) {
        return {
            status: 400,
...
export const getInventoryTransactions = async (): Promise<ServiceResult<unknown>> => {
...
export const getMyInventoryTransactions = async (userId: number): Promise<ServiceResult<unknown>> => {
    const records = await prisma.inventoryTransaction.findMany({
...
export const getRawMaterialsStock = async (): Promise<ServiceResult<unknown>> => {
