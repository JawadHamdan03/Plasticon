...
const router = Router();
// Only SALES_REP can create/delete; admin and accountant are read-only viewers
...
// Quotations — only SALES_REP can create/delete; SALES_REP+ACCOUNTANT can change status
...
router.delete("/quotations/:id",       authorizeRoles(salesRoles),         deleteQuotationHandler);

// Customer visits — only SALES_REP can create/delete; everyone can view
router.post("/visits",       authorizeRoles(salesRoles),  createVisitHandler);
router.get("/visits",        authorizeRoles(reviewRoles), getVisits);
router.delete("/visits/:id", authorizeRoles(salesRoles),  deleteVisitHandler);
