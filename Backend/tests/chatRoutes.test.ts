...
const { controllerMocks } = vi.hoisted(() => ({
    controllerMocks: {
        ...
        addGroupMemberHandler: vi.fn((req: Request, res: Response) =>
            res.status(201).json({ route: "add-member", groupId: req.params.groupId })
        ),
        removeGroupMemberHandler: vi.fn((req: Request, res: Response) =>
            res.status(200).json({ route: "remove-member", userId: req.params.userId })
        ),
        sendGroupMessageHandler: vi.fn((req: Request, res: Response) =>
            res.status(201).json({ route: "send-message", content: req.body.content })
        ),
        getGroupMessagesHandler: vi.fn((req: Request, res: Response) =>
            res.status(200).json({ route: "get-messages", limit: req.query.limit ?? null })
        ),
        markGroupAsReadHandler: vi.fn((req: Request, res: Response) =>
            res.status(200).json({ route: "mark-as-read", groupId: req.params.groupId })
        ),
    },
}));
...
describe("chatRoutes endpoints", () => {
    ...
    it("POST /chat/groups/:groupId/members", async () => {
        const res = await request(app).post("/chat/groups/4/members").send({ userId: 11 });
    ...
    it("POST /chat/groups/:groupId/messages", async () => {
        ...
        expect(res.status).toBe(201);
        expect(res.body).toEqual({ route: "send-message", content: "hello from test" });
    ...
    it("PATCH /chat/groups/:groupId/mark-as-read", async () => {
        const res = await request(app).patch("/chat/groups/8/mark-as-read").send({});
